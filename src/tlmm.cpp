...
Type objective_function<Type>::operator() ()
{
  ...
  // Wishart(nu = DEGREES, A*I) prior. The sum of the log-diagonal of L is
  // (1/2)log|Sigma|, so the first term is ((nu - 4)/2) log|Sigma|, the Wishart
  // exponent for p = 3.
  nll -= (DEGREES - Type(4.0)) * (ALPHA(0) + ALPHA(2) + ALPHA(5));
  nll += Sigma.trace() / (Type(2.0) * SCALE);        // tr(Sigma) / (2A)
